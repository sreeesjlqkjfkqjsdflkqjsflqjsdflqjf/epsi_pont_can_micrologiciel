...
eERRORRESULT Init_MCP251XFD(MCP251XFD *pComp, const MCP251XFD_Config *pConf) {
  ...
  //--- Check configuration ---------------------------------
  if ((pConf->XtalFreq != 0) && (pConf->XtalFreq < MCP251XFD_XTALFREQ_MIN))
    return ERR__FREQUENCY_ERROR; // The device crystal should not be <  4MHz
  if ((pConf->XtalFreq != 0) && (pConf->XtalFreq > MCP251XFD_XTALFREQ_MAX))
    return ERR__FREQUENCY_ERROR; // The device crystal should not be > 40MHz
  if ((pConf->OscFreq != 0) && (pConf->OscFreq < MCP251XFD_OSCFREQ_MIN))
    return ERR__FREQUENCY_ERROR; // The device oscillator should not be <  2MHz
  if ((pConf->OscFreq != 0) && (pConf->OscFreq > MCP251XFD_OSCFREQ_MAX))
    return ERR__FREQUENCY_ERROR; // The device oscillator should not be > 40MHz
  uint32_t CompFreq = 0;
  if (pConf->XtalFreq != 0)
  ...
  if (CompFreq == 0)
    return ERR__CONFIGURATION; // Both XtalFreq and OscFreq are configured to 0

  //--- Configure SPI Interface -----------------------------
  if (pComp->SPIClockSpeed > MCP251XFD_SPICLOCK_MAX)
    return ERR__SPI_FREQUENCY_ERROR; // The SPI clock should not be superior to
                                     // the SPI clock max
  ...
  //--- Test SPI connection ---------------------------------
  Error =
      MCP251XFD_WriteRAM32(pComp, (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE - 4),
                           0xAA55AA55); // Write 0xAA55AA55 at address
  if (Error != ERR_NONE)
    return Error; // If there is an error while writing the RAM address then
                  // return the error
  Error =
      MCP251XFD_ReadRAM32(pComp, (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE - 4),
                          &Result); // Read again the data
  if ((Error == ERR__CRC_ERROR) || (Result != 0xAA55AA55))
    return ERR__NO_DEVICE_DETECTED; // If CRC mismatch or data read is not
                                    // 0xAA55AA55 then no device is detected
  if (Error != ERR_NONE)
    return Error; // If there is an error while reading the RAM address then
                  // return the error
  ...
  if (CompFreq > MCP251XFD_CLKINPLL_MAX)
    return ERR__FREQUENCY_ERROR; // If component clock (CLKIN+PLL) is too high
  ...
  if ((pConf->SysclkConfig == MCP251XFD_SYSCLK_IS_CLKIN_DIV_2) ||
    ...
    CompFreq /= 2;
  } // Configure CLKIN+PLL divisor from Xtal/Resonator and PLL or oscillator
    // frequency
  if (pConf->SYSCLK_Result != NULL)
    *pConf->SYSCLK_Result = CompFreq; // Save the internal SYSCLK if needed
  if (CompFreq > MCP251XFD_SYSCLK_MAX)
    return ERR__FREQUENCY_ERROR; // If component clock (SYSCLK) is too high then
                                 // return an error
  if (pConf->ClkoPinConfig != MCP251XFD_CLKO_SOF)
    Config |= MCP251XFD_SFR_OSC8_CLKODIV_SET(
        pConf->ClkoPinConfig); // Configure the CLKO pin (CLKIN+PLL div by 1, 2,
                               // 4, 10 or Start Of Frame)
  Config |= MCP251XFD_SFR_OSC8_LPMEN; // Set now the Low Power Mode for further
                                      // check of which module MCP251XFD it is
  Error = MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_OSC_CONFIG,
      Config); // Write the Oscillator Register configuration
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_WriteSFR8()
                  // then return the error

  //--- Check clocks stabilization --------------------------
  uint8_t CheckVal =
      ((uint8_t)(Config)&MCP251XFD_SFR_OSC8_CHECKFLAGS) |
      MCP251XFD_SFR_OSC8_OSCRDY; // Check if PLL Locked (if enabled), OSC clock
                                 // is running and stable, and SCLKDIV is
                                 // synchronized (if divided by 2)
  uint32_t StartTime = pComp->fnGetCurrentms(); // Start the timeout
  while (true) {
    Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_OSC_CHECK,
                               &Config); // Read current OSC register mode with
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                    // then return the error
    ...
    if (MCP251XFD_TIME_DIFF(StartTime, pComp->fnGetCurrentms()) > 4)
      return ERR__DEVICE_TIMEOUT; // Wait at least 3ms (see TOSCSTAB in Table
                                  // 7-3 from datasheet Electrical
                                  // Specifications) + 1ms because GetCurrentms
                                  // can be 1 cycle before the new ms. If
                                  // timeout then return the error
  }

  //--- Set desired SPI clock speed -------------------------
  if (pComp->SPIClockSpeed > (((CompFreq >> 1) * 85) / 100))
    return ERR__SPI_FREQUENCY_ERROR; // Ensure that FSCK is less than or equal
                                     // to 0.85*(FSYSCLK/2). Follows datasheets
                                     // errata for: The SPI can write corrupted
                                     // data to the RAM at fast SPI speeds
  if ((pComp->DriverConfig & MCP251XFD_DRIVER_SAFE_RESET) > 0) {
    Error = pComp->fnSPI_Init(
        pComp->InterfaceDevice, pComp->SPI_ChipSelect,
        pComp->SPIClockSpeed); // Set the SPI speed clock to desired clock speed
    if (Error != ERR_OK)
      return Error; // If there is an error while changing SPI interface speed
                    // then return the error
  }
  ...
  {
    Error =
        __MCP251XFD_TestRAM(pComp); // Check the all the RAM only of the device
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling __MCP251XFD_TestRAM()
                    // then return the error
  } else            // Else check only SPI interface
  {
    Error = MCP251XFD_WriteRAM32(pComp,
                                 (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE - 4),
                                 0xAA55AA55); // Write 0xAA55AA55 at address
    if (Error != ERR_NONE)
      return Error; // If there is an error while writing the RAM address then
                    // return the error
    Error = MCP251XFD_ReadRAM32(pComp,
                                (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE - 4),
                                &Result); // Read again the data
    if (Error != ERR_NONE)
      return Error; // If there is an error while reading the RAM address then
    ...
    if (Result != 0xAA55AA55)
      return ERR__RAM_TEST_FAIL; // If data read is not 0xAA55AA55 then return
                                 // an error
  }
  ...
  {
    Error = MCP251XFD_ConfigureECC(
        pComp, true, MCP251XFD_ECC_ALL_EVENTS,
        0x55); // Configure the ECC and enable all interrupts related to ECC
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling
      ...
      0) // If there is a DRIVER_INIT_SET_RAM_AT_0 flag then
  {
    ...
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_InitRAM()
                    // then return the error
  }

  //--- Initialize Int pins or GPIOs ------------------------
  Error = MCP251XFD_SetGPIOPinsOutputLevel(
      pComp, pComp->GPIOsOutLevel,
      MCP251XFD_GPIO0_Mask |
          MCP251XFD_GPIO1_Mask); // Set GPIO pins output level before change to
                                 // mode GPIO. This is to get directly the good
                                 // output level when (if) pins will be in
                                 // output mode
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_ConfigurePins() then return the error
  Error = MCP251XFD_ConfigurePins(
      pComp, pConf->GPIO0PinMode, pConf->GPIO1PinMode, pConf->INTsOutMode,
      pConf->TXCANOutMode,
  ...
  Error = MCP251XFD_CalculateBitTimeConfiguration(
      CompFreq, pConf->NominalBitrate, pConf->DataBitrate,
      &ConfBitTime); // Calculate Bit Time
  if (Error != ERR_OK)
    return Error; // If there is an error while calling
                  // MCP251XFD_CalculateBitTimeConfiguration() then return the
                  // error
  ...
  if (Error != ERR_OK)
    return Error; // If there is an error while calling
                  // MCP251XFD_SetBitTimeConfiguration() then return the error

  //--- CAN configuration -----------------------------------
  Error = MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_CiCON + 2,
  ...
  Error = MCP251XFD_ConfigureCANController(
      pComp, pConf->ControlFlags,
      pConf->Bandwidth); // Configure the CAN control
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_ConfigureCANControl() then return the error

  //--- System interrupt enable -----------------------------
  Error = MCP251XFD_ConfigureInterrupt(
      pComp, (setMCP251XFD_InterruptEvents)
...
eERRORRESULT MCP251XFD_InitRAM(MCP251XFD *pComp) {
  ...
  for (uint16_t Address = MCP251XFD_RAM_ADDR;
       Address < (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE); Address += 4) {
    Error = MCP251XFD_WriteRAM32(pComp, Address,
                                 0x00000000); // Write 0x00000000 at address
    if (Error != ERR_NONE)
      return Error; // If there is an error while writing the RAM address then
                    // return the error
  }
...
//=============================================================================
eERRORRESULT MCP251XFD_GetDeviceID(MCP251XFD *pComp, eMCP251XFD_Devices *device,
                                   uint8_t *deviceId, uint8_t *deviceRev) {
#ifdef CHECK_NULL_PARAM
  if ((pComp == NULL) || (device == NULL))
    return ERR__PARAMETER_ERROR;
#endif
  *device = (eMCP251XFD_Devices)MCP251XFD_DEV_ID_GET(
      pComp->InternalConfig); // Get device found when initializing the module
  eERRORRESULT Error;

  if ((deviceId != NULL) || (deviceRev != NULL)) {
    uint8_t Value;
    Error = MCP251XFD_ReadSFR8(
        pComp, RegMCP251XFD_DEVID,
    ...
    if (deviceRev != NULL)
      *deviceRev = MCP251XFD_SFR_DEVID8_REV_GET(Value); // Get Device Revision
  }
  return ERR_NONE;
}
...
//=============================================================================
eERRORRESULT MCP251XFD_ReadData(MCP251XFD *pComp, uint16_t address,
  ...
  const bool InRAM =
      ...
      (address <
       (MCP251XFD_RAM_ADDR +
        MCP251XFD_RAM_SIZE)); // True if address is in the RAM region ; False if
                              // address is in Controller SFR or CAN SFR region
  ...
  uint16_t Increment =
      ...
      (UseCRC
           ? 5
           : 2); // If use CRC for read, Buffer size minus 2 for Command, minus
                 // 1 for length, minus 2 for CRC, else just 2 for command
  if (UseCRC && InRAM) // In RAM region and use CRC for read?
  {
    if ((size & 0b11) != 0)
      return ERR__DATA_MODULO; // size should be a multiple of 4 in case of
                               // Write CRC or Safe Write
    Increment &= 0xFFFC; // If in RAM region then the increment should be the
                         // nearest less than or equal value multiple by 4
  } // Here Increment cannot be 0 because MCP251XFD_TRANS_BUF_SIZE is compiler
    // protected to be not less than 9
  ...
  while (size > 0) {
    const uint16_t Addr =
        MCP251XFD_SPI_16BITS_WORD((UseCRC ? MCP251XFD_SPI_INSTRUCTION_READ_CRC
    ...
    *pBuf = Addr & 0xFF; // Set next byte of SPI command
    ...
    if ((pComp->DriverConfig & MCP251XFD_DRIVER_CLEAR_BUFFER_BEFORE_READ) > 0) {
      const size_t BuffUsedSize =
          ByteCount +
          (UseCRC ? 2 + 1 - 2
                  : 2); // Here 2 for Command + 1 for Length - 2 for CRC (CRC on
                        // the SPI send is not checked by the controller thus
                        // will be set to 0 too), else just 2 for command
    ...
    uint8_t LenData = 0;
    if (UseCRC && InRAM)
      LenData = (ByteCount >> 2) & 0xFF; // If use CRC for read and in RAM, set
                                         // how many data word that is requested
    else
    ...
    const size_t ByteToReadCount =
        ...
        (UseCRC
             ? (2 + 1 + 2)
             : 2); // In case of use CRC for read, here are 2 bytes for Command
                   // + 1 for Length + 2 for CRC, else just 2 for command
    Error = pComp->fnSPI_Transfer(
        pComp->InterfaceDevice, pComp->SPI_ChipSelect, &Buffer[0], &Buffer[0],
        ByteToReadCount); // Transfer the data in the buffer
    if (Error != ERR_NONE)
      return Error; // If there is an error while transferring data then return
                    // the error
    ...
    if (UseCRC) {
      ...
      if (pComp->fnComputeCRC16 == NULL)
        return ERR__PARAMETER_ERROR; // If the CRC function is not present,
                                     // raise an error
#endif
      uint16_t CRC = pComp->fnComputeCRC16(
          &Buffer[0],
          (ByteCount + 2 +
           1)); // Compute CRC on the Buffer data (2 for Command + 1 for Length)
      uint16_t BufCRC = *pBuf << 8; // Get CRC MSB on the next buffer byte
      ++pBuf;
      BufCRC |= *pBuf; // Get CRC LSB on the next buffer byte
      if (CRC != BufCRC)
        return ERR__CRC_ERROR; // If CRC mismatch, then raise an error
    }
  }
...
//=============================================================================
eERRORRESULT MCP251XFD_WriteData(MCP251XFD *pComp, uint16_t address,
  ...
  if (UseSafe == false) {
    Increment =
        ...
        (UseCRC
             ? 5
             : 2); // If use CRC for writeBuffer size minus 2 for Command, minus
                   // 1 for length, minus 2 for CRC, else just 2 for command
    ...
    Instruction =
        MCP251XFD_SPI_INSTRUCTION_SAFE_WRITE; // Set safe write instruction
    if (InRAM) {
      if ((size & 0b11) != 0)
        return ERR__DATA_MODULO; // size should be a multiple of 4 in case of
                                 // Write CRC or Safe Write
      Increment =
          4; // If use safe write and in RAM then the increment is 4 bytes
    } else
      Increment =
          1; // If use safe write and in SFR then the increment is 1 byte
  }
  ...
  while (size > 0) {
    ...
    *pBuf = Addr & 0xFF; // Set next byte of SPI command
    ...
    ByteCount =
        (size > Increment ? Increment : size); // Define byte count to send
    if (UseCRC &&
        (UseSafe == false)) // Add Len in the frame if use CRC but not use safe
    {
      if (InRAM)
        *pBuf = (ByteCount >> 2) & 0xFF; // If use CRC for write and in RAM, set
                                         // how many data word that is requested
      else
    ...
    if (UseCRC) {
      ...
      uint16_t FrameCRC = pComp->fnComputeCRC16(
          &Buffer[0],
          ByteToComputeCount); // Compute CRC on the Buffer data (2 for Command
                               // + 1 for Length for not safe)
    ...
    Error = pComp->fnSPI_Transfer(
        pComp->InterfaceDevice, pComp->SPI_ChipSelect, &Buffer[0], NULL,
        ByteToWriteCount); // Transfer the data in the buffer (2 for Command + 1
                           // for Length + 2 for CRC)
    if (Error != ERR_NONE)
      return Error; // If there is an error while transferring data then return
                    // the error
  }
...
//=============================================================================
eERRORRESULT MCP251XFD_TransmitMessageObjectToFIFO(MCP251XFD *pComp,
  ...
  Error = MCP251XFD_GetNextMessageAddressFIFO(pComp, toFIFO, &NextAddress,
                                              NULL); // Get next message address
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_GetNextMessageAddressFIFO() then return the error
  NextAddress += MCP251XFD_RAM_ADDR;

  //--- Write data to RAM ---
  Error = MCP251XFD_WriteData(pComp, NextAddress, &messageObjectToSend[0],
                              objectSize); // Write data to RAM address
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_WriteData()
                  // then return the error
...
MCP251XFD_TransmitMessageToFIFO(MCP251XFD *pComp,
                                ...
                                eMCP251XFD_FIFO toFIFO, bool andFlush) {
#ifdef CHECK_NULL_PARAM
  ...
  MCP251XFD_CAN_TX_Message *Message = (MCP251XFD_CAN_TX_Message *)
      Buffer; // The first 8 bytes represent the MCP251XFD_CAN_TX_Message struct
  ...
  Message->T0.T0 = MCP251XFD_MessageIDtoObjectMessageIdentifier(
      messageToSend->MessageID, Extended, MCP251XFD_USE_SID11 && CANFDframe);
  ...
  if ((messageToSend->ControlFlags & MCP251XFD_SWITCH_BITRATE) > 0)
    Message->T1.BRS = 1;
  if ((messageToSend->ControlFlags & MCP251XFD_REMOTE_TRANSMISSION_REQUEST) > 0)
    Message->T1.RTR = 1;
  if ((messageToSend->ControlFlags & MCP251XFD_EXTENDED_MESSAGE_ID) > 0)
    Message->T1.IDE = 1;
  if ((messageToSend->ControlFlags & MCP251XFD_TRANSMIT_ERROR_PASSIVE) > 0)
    Message->T1.ESI = 1;
  ...
  if (messageToSend->PayloadData != NULL) {
    uint8_t *pBuff = &Buffer[sizeof(
        MCP251XFD_CAN_TX_Message)]; // Next bytes of the Buffer is for payload
    uint8_t *pData =
        &messageToSend->PayloadData[0]; // Select the first byte of payload data
    uint8_t BytesToCopy =
  ...
  if ((BytesToSend & 0x3) != 0)
    BytesToSend =
        (BytesToSend & 0xFC) +
        4; // Adjust to the upper modulo 4 bytes (mandatory for RAM access)
  return MCP251XFD_TransmitMessageObjectToFIFO(pComp, &Buffer[0], BytesToSend,
                                               toFIFO, andFlush);
}
...
//=============================================================================
eERRORRESULT MCP251XFD_ReceiveMessageObjectFromFIFO(MCP251XFD *pComp,
  ...
  //--- Get address where to write the frame ---
  uint32_t NextAddress = 0;
  Error = MCP251XFD_GetNextMessageAddressFIFO(pComp, fromFIFO, &NextAddress,
                                              NULL); // Get next message address
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_GetNextMessageAddressFIFO() then return the error
  ...
  //--- Read data from RAM ---
  Error = MCP251XFD_ReadData(pComp, NextAddress, &messageObjectGet[0],
                             objectSize); // Read data to RAM address
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadData()
                  // then return the error

  //--- Update FIFO ---
  return MCP251XFD_UpdateFIFO(pComp, fromFIFO,
                              false); // Can't flush a receive FIFO
}
...
eERRORRESULT MCP251XFD_ReceiveMessageFromFIFO(
    MCP251XFD *pComp, MCP251XFD_CANMessage *messageGet,
    eMCP251XFD_PayloadSize payloadSize, uint32_t *timeStamp,
    eMCP251XFD_FIFO fromFIFO) {
#ifdef CHECK_NULL_PARAM
  ...
  MCP251XFD_CAN_RX_Message *Message = (MCP251XFD_CAN_RX_Message *)
      Buffer; // The first 8 bytes represent the MCP251XFD_CAN_RX_Message struct
  eERRORRESULT Error;
  ...
  if (fromFIFO == MCP251XFD_TEF)
    BytesToGet = sizeof(
        MCP251XFD_CAN_TX_EventObject); // In case of TEF, there is not data
  if (timeStamp == NULL)
    BytesToGet -= sizeof(uint32_t); // Time Stamp not needed = 4 bytes less
  if ((BytesToGet & 0x3) != 0)
    BytesToGet =
        (BytesToGet & 0xFC) +
        4; // Adjust to the upper modulo 4 bytes (mandatory for RAM access)
  ...
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_ReceiveMessageObjectFromFIFO() then return the
                  // error
  ...
  if (fromFIFO == MCP251XFD_TEF)
    messageGet->MessageSEQ =
        ((MCP251XFD_CAN_TX_EventObject *)Buffer)
            ->TE1.SEQ; // If it is a TEF, extract the Sequence by casting Buffer
                       // into a TEF object and get SEQ in TE1
  if (CANFDframe)
  ...
  uint8_t *pBuff =
      &Buffer[sizeof(MCP251XFD_CAN_RX_Message_Identifier) +
              sizeof(
                  MCP251XFD_CAN_RX_Message_Control)]; // Next bytes of the
                                                      // Buffer is for timestamp
                                                      // and/or payload
  if (timeStamp != NULL) {
  ...
  if (fromFIFO != MCP251XFD_TEF) {
    ...
    if (messageGet->PayloadData != NULL) {
      uint8_t *pData =
          &messageGet->PayloadData[0]; // Select the first byte of payload data
      uint8_t BytesDLC = MCP251XFD_DLCToByte(
          messageGet->DLC,
          CANFDframe); // Get how many byte need to be extract from the message
                       // to correspond to its DLC
      if (BytesPayload < BytesDLC)
        BytesDLC =
            BytesPayload; // Get the least between BytesPayload and BytesDLC
      while (BytesDLC-- > 0)
...
eERRORRESULT MCP251XFD_ConfigureCRC(MCP251XFD *pComp,
  ...
  return MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_CRC_CONFIG,
      interrupts); // Write configuration to the CRC register (last byte only)
}
...
eERRORRESULT MCP251XFD_GetCRCEvents(MCP251XFD *pComp,
  ...
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CRC_FLAGS,
      (uint8_t *)events); // Read status of the CRC register (third byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                  // then return the error
  *events =
  ...
  if (lastCRCMismatch != NULL) {
    Error = MCP251XFD_ReadSFR16(
        pComp, RegMCP251XFD_CRC_CRC,
        lastCRCMismatch); // Read Address where last CRC error occurred (first 2
                          // bytes only)
  }
  return Error;
}
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureECC(MCP251XFD *pComp, bool enableECC,
  ...
  Config[0] |= (uint8_t)interrupts; // Single error correction and double error
                                    // detection interrupts enable
  if (enableECC)
    Config[0] |= MCP251XFD_SFR_ECCCON8_ECCEN; // If ECC enable then add the ECC
                                              // enable flag
  Config[1] = MCP251XFD_SFR_ECCCON8_PARITY_SET(
      fixedParityValue); // Add the fixed parity used during write to RAM when
                         // ECC is disabled
  return MCP251XFD_WriteData(
      pComp, RegMCP251XFD_ECCCON, &Config[0],
...
eERRORRESULT MCP251XFD_GetECCEvents(MCP251XFD *pComp,
  ...
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_ECCSTAT_FLAGS,
      (uint8_t *)events); // Read status of the ECC register (first bytes only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                  // then return the error
  if (lastErrorAddress != NULL) {
    Error = MCP251XFD_ReadSFR16(
        pComp, RegMCP251XFD_ECCSTAT_ERRADDR,
        lastErrorAddress); // Read Address where last ECC error occurred (last 2
                           // bytes only)
  }
  return Error;
}
...
eERRORRESULT MCP251XFD_ConfigurePins(MCP251XFD *pComp,
  ...
  if ((GPIO0PinMode != MCP251XFD_PIN_AS_INT0_TX) &&
    ...
    Config |=
        MCP251XFD_SFR_IOCON8_GPIO0_MODE; // If the pin INT0/GPIO0/XSTBY is in
                                         // GPIO mode then set GPIO mode
  if ((GPIO1PinMode != MCP251XFD_PIN_AS_INT1_RX))
    Config |= MCP251XFD_SFR_IOCON8_GPIO1_MODE; // If the pin INT1/GPIO1 is in
                                               // GPIO mode then set GPIO mode
  if (TXCANOutMode == MCP251XFD_PINS_OPENDRAIN_OUT)
    Config |=
        MCP251XFD_SFR_IOCON8_TXCANOD; // If the pin TXCAN mode is open drain
                                      // mode then set open drain output
  if (CLKOasSOF)
    Config |= MCP251XFD_SFR_IOCON8_SOF; // If the pin CLKO/SOF is in SOF mode
                                        // then set SOF signal on CLKO pin
  if (INTOutMode == MCP251XFD_PINS_OPENDRAIN_OUT)
    Config |=
        MCP251XFD_SFR_IOCON8_INTOD; // If all interrupt pins mode are open drain
                                    // mode then set open drain output
  Error = MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_IOCON_PINMODE,
  ...
  if (GPIO0PinMode == MCP251XFD_PIN_AS_XSTBY)
    Config |= MCP251XFD_SFR_IOCON8_XSTBYEN; // If the pin INT0/GPIO0/XSTBY is in
                                            // XSTBY mode then enable XSTBY mode
  if (GPIO0PinMode == MCP251XFD_PIN_AS_GPIO0_IN)
    Config |= MCP251XFD_SFR_IOCON8_GPIO0_INPUT; // If the pin INT0/GPIO0/XSTBY
                                                // is in GPIO input mode then
                                                // set GPIO input mode
  if (GPIO1PinMode == MCP251XFD_PIN_AS_GPIO1_IN)
    Config |=
        MCP251XFD_SFR_IOCON8_GPIO1_INPUT; // If the pin INT1/GPIO1 is in GPIO
                                          // input mode then set GPIO input mode
  return MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_IOCON_DIRECTION,
      Config); // Write configuration to the IOCON register (first byte only)
}
...
eERRORRESULT MCP251XFD_SetGPIOPinsDirection(MCP251XFD *pComp,
  ...
  pinsChangeMask &= 0x3;
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_IOCON_DIRECTION,
                             &Config); // Read actual configuration of the IOCON
  ...
  return MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_IOCON_DIRECTION,
                             Config); // Write new configuration to the IOCON
                                      // register (first byte only)
}
...
eERRORRESULT MCP251XFD_GetGPIOPinsInputLevel(MCP251XFD *pComp,
  ...
  return MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_IOCON_INLEVEL,
      pinsState); // Read actual state of the input pins in the IOCON register
                  // (third byte only)
}
...
eERRORRESULT MCP251XFD_SetGPIOPinsOutputLevel(MCP251XFD *pComp,
  ...
  return MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_IOCON_OUTLEVEL,
      pComp->GPIOsOutLevel); // Write new configuration to the IOCON register
                             // (Second byte only)
}

//**********************************************************************************************************************************************************
//=============================================================================
// Calculate Bit Time for CAN2.0 or CAN-FD configuration for the MCP251XFD
// device
//=============================================================================
eERRORRESULT MCP251XFD_CalculateBitTimeConfiguration(
    const uint32_t fsysclk, const uint32_t desiredNominalBitrate,
    const uint32_t desiredDataBitrate, MCP251XFD_BitTimeConfig *pConf) {
#ifdef CHECK_NULL_PARAM
  if (pConf == NULL)
    return ERR__PARAMETER_ERROR;
#endif
  ...
  uint32_t BestBRP = MCP251XFD_NBRP_MAX, BestNTQbits = MCP251XFD_NTQBIT_MAX,
           BestDTQbits = MCP251XFD_DTQBIT_MAX;

  //--- Calculate Nominal & Data Bit Time parameter ---------
  uint32_t MinErrorBR = UINT32_MAX;
  uint32_t BRP =
      MCP251XFD_NBRP_MAX; // Select the worst BRP value. Here all value from max
                          // to min will be tested to get the best tuple of NBRP
                          // and DBRP, identical TQ in both phases prevents
                          // quantization errors during bit rate switching
  while (--BRP >= MCP251XFD_NBRP_MIN) {
    uint32_t NTQbits = fsysclk / desiredNominalBitrate /
                       BRP; // Calculate the NTQbits according to BRP and the
                            // desired Nominal Bitrate
    if ((NTQbits < MCP251XFD_NTQBIT_MIN) || (NTQbits > MCP251XFD_NTQBIT_MAX))
      continue; // This TQbits count is not possible with this BRP, then do the
                // next BRP value
    if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
      DTQbits = fsysclk / desiredDataBitrate /
                BRP; // Calculate the DTQbits according to BRP and the desired
                     // Data Bitrate
      if ((DTQbits < MCP251XFD_DTQBIT_MIN) || (DTQbits > MCP251XFD_DTQBIT_MAX))
        continue; // This TQbits count is not possible with this BRP, then do
                  // the next BRP value
    }

    // NTQ & DTQ bits count
    ErrorNTQ = (fsysclk -
                (desiredNominalBitrate * NTQbits * BRP)); // Calculate NTQ error
    if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
      if (ErrorNTQ == 0)
  ...
  uint32_t NTSEG2 =
      BestNTQbits / 5; // The Nominal Sample Point must be close to 80% (5x20%)
                       // of NTQ per bits so NTSEG2 should be 20% of NTQbits
  if ((BestNTQbits % 5) > 2)
  ...
  uint32_t NTSEG1 = BestNTQbits - NTSEG2 -
  ...
  uint32_t NSJW = NTSEG2; // Normally NSJW = NTSEG2, maximizing NSJW lessens the
  ...
  if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
    ...
    uint32_t DTSEG2 =
        BestDTQbits / 5; // The Data Sample Point must be close to 80% (5x20%)
                         // of DTQ per bits so DTSEG2 should be 20% of DTQbits
    if ((BestDTQbits % 5) > 2)
    ...
    uint32_t DTSEG1 = BestDTQbits - DTSEG2 -
    ...
    uint32_t DSJW = DTSEG2; // Normally DSJW = DTSEG2, maximizing DSJW lessens
                            // the requirement for the oscillator tolerance
    if (DTSEG1 < DTSEG2)
    ...
    if (desiredDataBitrate >=
      ...
      pConf->TDCMOD =
          MCP251XFD_AUTO_MODE; // ** Set Automatic TDC measurement compensations
                               // for transmitter delay variations
    else
      pConf->TDCMOD = MCP251XFD_MANUAL_MODE; // ** Set Manual; Don�t measure,
                                             // use TDCV + TDCO from register
    const uint32_t SSP =
        BestBRP *
        DTSEG1; // In order to set the SSP to 80%, SSP = TDCO + TDCV (Equation
                // 3-10 of MCP25XXFD Family Reference Manual). SSP is set to
                // DBRP * (DPRSEG + DPHSEG1) = DBRP * DTSEG1
    uint32_t TDCO = SSP;
    if (TDCO > MCP251XFD_TDCO_MAX)
      TDCO = MCP251XFD_TDCO_MAX; // Correct TDCO if > 63
    pConf->TDCO = TDCO;          // ** Save the TDCO in the configuration **
    uint32_t TDCV =
        SSP - TDCO; // TDCV is the remaining of SSP: TDCV = SSP - TDCO (Equation
    ...
    pConf->EDGE_FILTER =
        true; // ** Edge Filtering enabled, according to ISO 11898-1:2015 **
  } else {
  ...
  if (pConf->Stats != NULL)
    Error = MCP251XFD_CalculateBitrateStatistics(
        fsysclk, pConf,
        desiredDataBitrate ==
            MCP251XFD_NO_CANFD); // If statistics are necessary, then calculate
                                 // them
  return Error;                  // If there is an error while calling
                // MCP251XFD_CalculateBitrateStatistics() then return the error
}
...
eERRORRESULT MCP251XFD_CalculateBitrateStatistics(
    const uint32_t fsysclk, MCP251XFD_BitTimeConfig *pConf, bool can20only) {
#ifdef CHECK_NULL_PARAM
  if (pConf == NULL)
    return ERR__PARAMETER_ERROR;
  if (pConf->Stats == NULL)
  ...
  const uint32_t NTQ =
      (((pConf->NBRP + 1) * 1000000) /
       (fsysclk /
        1000)); // Nominal Time Quanta = 1/FSYSCLK multiply by 1000000000 to get
                // ns (Equation 3-3 of MCP25XXFD Family Reference Manual)
  const uint32_t NPRSEG =
      (pConf->NTSEG1 + 1) -
      (pConf->NTSEG2 +
       1); // Here PHSEG2 (NTSEG2) should be equal to PHSEG1 so NPRSEG = NTSEG1
           // - NTSEG2 (Figure 3-2 of MCP25XXFD Family Reference Manual)
  pConf->Stats->MaxBusLength =
      (uint32_t)(((NTQ * NPRSEG) - (2 * MCP251XFD_tTXDtRXD_MAX)) /
                 (2 *
                  MCP251XFD_tBUS_CONV)); // Formula is (2x(tTXD�RXD +
                                         // (5*BusLen))/NTQ = NPRSEG => BusLen =
                                         // ((NTQ*NPRESG)-(2*tTXD))/(2*5) in
                                         // meter (Equation 3-9 of MCP25XXFD
                                         // Family Reference Manual)
  const uint32_t NTQbits =
      (MCP251XFD_NSYNC + (pConf->NTSEG1 + 1) +
       (pConf->NTSEG2 + 1)); // NTQ per bits = NSYNC + NTSEG1 + NTSEG2 (Equation
                             // 3-5 of MCP25XXFD Family Reference Manual)
  ...
  pConf->Stats->NSamplePoint =
      (uint32_t)(SamplePoint * 100); // ** Save actual Nominal sample point with
                                     // 2 digits after the decimal point (divide
                                     // by 100 to get percentage)
  pConf->Stats->NominalBitrate =
      (fsysclk / (pConf->NBRP + 1) / NTQbits); // ** Save actual Nominal Bitrate

  //--- Calculate Data Sample Point -------------------------
  if (can20only == false) {
    DTQbits = (MCP251XFD_DSYNC + (pConf->DTSEG1 + 1) +
               (pConf->DTSEG2 +
                1)); // DTQ per bits = DSYNC + DTSEG1 + DTSEG2 (Equation 3-6 of
    ...
    pConf->Stats->DSamplePoint =
        (uint32_t)(SamplePoint *
                   100.0f); // ** Save actual Data sample point with 2 digits
                            // after the decimal point (divide by 100 to get
                            // percentage)
    pConf->Stats->DataBitrate =
        (fsysclk / (pConf->DBRP + 1) / DTQbits); // ** Save actual Data Bitrate
  } else {
  ...
  pConf->Stats->OscTolC1 =
      (((pConf->NSJW + 1) * 10000) /
       (2 * 10 *
        NTQbits)); // Condition 1 for the maximum tolerance of the oscillator
                   // with 2 digits after the decimal point (Equation 3-12 of
                   // MCP25XXFD Family Reference Manual)
  ...
  pConf->Stats->OscTolC2 =
      ((MinNPHSEG * 10000) /
       (2 * (13 * NTQbits -
             (pConf->NTSEG2 +
              1)))); // Condition 2 for the maximum tolerance of the oscillator
                     // with 2 digits after the decimal point (Equation 3-13 of
                     // MCP25XXFD Family Reference Manual)
  pConf->Stats->OscTolerance =
      (pConf->Stats->OscTolC2 < pConf->Stats->OscTolerance
           ? pConf->Stats->OscTolC2
           : pConf->Stats->OscTolerance); // Oscillator Tolerance, minimum of
  ...
  if (can20only) {
    ...
    pConf->Stats->OscTolC3 =
        (((pConf->DSJW + 1) * 10000) /
         (2 * 10 *
          DTQbits)); // Condition 3 for the maximum tolerance of the oscillator
                     // with 2 digits after the decimal point (Equation 3-14 of
                     // MCP25XXFD Family Reference Manual)
    pConf->Stats->OscTolerance =
    ...
    pConf->Stats->OscTolC4 =
        ((MinNPHSEG * 10000) /
         (2 *
          ((((6 * DTQbits - (pConf->DTSEG2 + 1)) * DBRP) / NBRP) +
           (7 *
            NTQbits)))); // Condition 4 for the maximum tolerance of the
        ...
        (pConf->Stats->OscTolC4 < pConf->Stats->OscTolerance
             ...
             : pConf->Stats
                   ->OscTolerance); // Oscillator Tolerance, minimum of
                                    // conditions 1-5 (Equation 3-11 of
                                    // MCP25XXFD Family Reference Manual)
    const int32_t NBRP_DBRP = ((NBRP * 10000) / DBRP),
                  MaxBRP =
                      ((NBRP_DBRP - 10000) > 0
                           ? (NBRP_DBRP - 10000)
    ...
    pConf->Stats->OscTolC5 =
        ((((pConf->DSJW + 1) * 10000) - MaxBRP) /
         (2 * (((2 * NTQbits - (pConf->NTSEG2 + 1)) * NBRP) / DBRP +
               (pConf->DTSEG2 + 1) +
               4 * DTQbits))); // Condition 5 for the maximum tolerance of the
                               ...
                               // Reference Manual) [WARNING: An error seems to
                               // be present in the original formula]
    pConf->Stats->OscTolerance =
...
//=============================================================================
eERRORRESULT MCP251XFD_SetBitTimeConfiguration(MCP251XFD *pComp,
  ...
  NConfig.CiNBTCFG = MCP251XFD_CAN_CiNBTCFG_BRP_SET(pConf->NBRP) |
  ...
  Error = MCP251XFD_WriteData(
      pComp, RegMCP251XFD_CiNBTCFG, &NConfig.Bytes[0],
  ...
  if (!can20only) {
    ...
    DConfig.CiDBTCFG = MCP251XFD_CAN_CiDBTCFG_BRP_SET(pConf->DBRP) |
    ...
    Error = MCP251XFD_WriteData(
        pComp, RegMCP251XFD_CiDBTCFG, &DConfig.Bytes[0],
        sizeof(MCP251XFD_CiDBTCFG_Register)); // Write configuration to the
                                              // CiDBTCFG register
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_WriteData()
                    // then return the error
    ...
    Error = MCP251XFD_WriteData(
        pComp, RegMCP251XFD_CiTDC, &TConfig.Bytes[0],
        sizeof(MCP251XFD_CiTDC_Register)); // Write configuration to the
                                           // CiDBTCFG register
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_WriteData()
    pComp->InternalConfig |=
        MCP251XFD_CANFD_ENABLED; // CAN-FD is enable if Data Bitrate is set
  } else
...
eERRORRESULT MCP251XFD_AbortAllTransmissions(MCP251XFD *pComp) {
  ...
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiCON + 3,
                             &Config); // Read actual configuration of the CiCON
                                       // register (Last byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                  // then return the error
  Config |= MCP251XFD_CAN_CiCON8_ABAT; // Add ABAT flag
  return MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_CiCON + 3,
...
MCP251XFD_GetActualOperationMode(MCP251XFD *pComp,
  ...
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiCON + 2,
                             &Config); // Read actual configuration of the CiCON
                                       // register (Third byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
...
//=============================================================================
eERRORRESULT MCP251XFD_RequestOperationMode(MCP251XFD *pComp,
  ...
  if (((pComp->InternalConfig & MCP251XFD_CANFD_ENABLED) == 0) &&
    ...
    return ERR__CONFIGURATION; // Can't change to CAN-FD mode if the bitrate is
                               // not configured for
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiCON + 3,
                             &Config); // Read actual configuration of the CiCON
                                       // register (Last byte only)
  ...
  Config |= MCP251XFD_CAN_CiCON8_ABAT; // Need to stop all transmissions before
                                       // changing configuration
  Error = MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_CiCON + 3,
                              Config); // Write the new configuration to the
                                       // CiCON register (Last byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_WriteSFR8()
  ...
  if (waitOperationChange) {
    ...
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling
                    // MCP251XFD_WaitOperationModeChange() then return the error
    Error = MCP251XFD_ClearInterruptEvents(
        pComp,
        MCP251XFD_INT_OPERATION_MODE_CHANGE_EVENT); // Automatically clear the
                                                    // Operation Mode Change
                                                    // Flag
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling
                    // MCP251XFD_ClearInterruptEvents() then return the error
  }
  pComp->InternalConfig &= ~MCP251XFD_DEV_PS_Mask;
  pComp->InternalConfig |= MCP251XFD_DEV_PS_SET(
      MCP251XFD_DEVICE_NORMAL_POWER_STATE); // Set normal power state even if
                                            // the operation mode is sleep, this
                                            // value will be changed to the good
...
MCP251XFD_WaitOperationModeChange(MCP251XFD *pComp,
  ...
  while (true) {
    Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiCON + 2,
                               &Config); // Read current configuration mode with
    ...
    if (MCP251XFD_TIME_DIFF(StartTime, pComp->fnGetCurrentms()) >
        7) // Wait at least 7ms because the longest message is 731 bit long and
           // the minimum bitrate is 125kbit/s that mean 5,8ms + 2x 6bytes @
           // 1Mbit/s over SPI that mean 96µs = ~6ms + 1ms because GetCurrentms
           // can be 1 cycle before the new ms
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureCANController(MCP251XFD *pComp,
                                              setMCP251XFD_CANCtrlFlags flags,
                                              eMCP251XFD_Bandwidth bandwidth) {
  eERRORRESULT Error;
  uint32_t Config;

  Error = MCP251XFD_ReadSFR32(pComp, RegMCP251XFD_CiCON,
                              &Config); // Read actual configuration of the
                                        // CiTDC register (Last byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR32()
                  // then return the error
  if (MCP251XFD_CAN_CiCON_OPMOD_GET(Config) != MCP251XFD_CONFIGURATION_MODE)
    return ERR__NEED_CONFIG_MODE; // Device must be in Configuration Mode to
                                  // perform the configuration

  Config &= ~(
      MCP251XFD_CAN_CiCON_TXBWS_Mask | MCP251XFD_CAN_CiCON_REQOP_Mask |
      MCP251XFD_CAN_CiCON_ABAT // Clear by default all flags that can be changed
  ...
  Config |= MCP251XFD_CAN_CiCON_REQOP_SET(
      MCP251XFD_CONFIGURATION_MODE); // Stay in configuration mode because these
                                     // change are done in configuration mode
                                     // only
  ...
  if ((flags & MCP251XFD_CAN_LISTEN_ONLY_MODE_ON_ERROR) > 0)
    Config |= MCP251XFD_CAN_CiCON_SERR2LOM; // Set transition to Listen Only
                                            // Mode on system error
  if ((flags & MCP251XFD_CAN_GATEWAY_MODE_ESI_RECESSIVE) > 0)
    Config |=
        MCP251XFD_CAN_CiCON_ESIGM; // Set transmit ESI in Gateway Mode, ESI is
                                   // transmitted recessive when ESI of message
                                   // is high or CAN controller error passive
  if ((flags & MCP251XFD_CAN_RESTRICTED_RETRANS_ATTEMPTS) > 0)
    Config |= MCP251XFD_CAN_CiCON_RTXAT; // Set restricted retransmission
                                         // attempts, CiFIFOCONm.TXAT is used
  if ((flags & MCP251XFD_CANFD_BITRATE_SWITCHING_DISABLE) > 0)
    Config |= MCP251XFD_CAN_CiCON_BRSDIS; // Set Bit Rate Switching is Disabled,
                                          // regardless of BRS in the Transmit
                                          // Message Object
  if ((flags & MCP251XFD_CAN_PROTOCOL_EXCEPT_AS_FORM_ERROR) > 0)
    Config |= MCP251XFD_CAN_CiCON_PXEDIS; // Set Protocol Exception is treated
  ...
  if ((flags & MCP251XFD_CANFD_USE_ISO_CRC) > 0)
    Config |= MCP251XFD_CAN_CiCON_ISOCRCEN; // Set Include Stuff Bit Count in
                                            // CRC Field and use Non-Zero CRC
                                            // Initialization Vector according
                                            // to ISO 11898-1:2015
  Error = MCP251XFD_WriteSFR32(
      pComp, RegMCP251XFD_CiCON,
      Config); // Write new configuration to the CiTDC register (Last byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_WriteSFR8()
                  // then return the error

  uint8_t TConfig;
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiTDC_CONFIG,
  ...
  if ((flags & MCP251XFD_CANFD_USE_RRS_BIT_AS_SID11) > 0) // Use SID11?
  {
    ...
    pComp->InternalConfig |=
        MCP251XFD_CANFD_USE_RRS_BIT_AS_SID11; // Add use SID11 to the internal
                                              // configuration
  } else
    pComp->InternalConfig &=
        ~MCP251XFD_CANFD_USE_RRS_BIT_AS_SID11; // Else clear use SID11 to the
                                               // internal configuration
  return MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_CiTDC_CONFIG,
                             TConfig); // Write new configuration to the CiTDC
                                       // register (Last byte only)
}
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureSleepMode(MCP251XFD *pComp,
  ...
  if (useLowPowerMode &&
    ...
    return ERR__NOT_SUPPORTED; // If the device is MCP2517FD then it does not
                               // support Low Power Mode
  eERRORRESULT Error;

  uint8_t Config;
  Error =
      MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_OSC_CONFIG,
                         &Config); // Read the Oscillator Register configuration
  if (Error != ERR_NONE)
    return Error; // If there is an error while reading the SFR then return the
                  // error
  ...
  Error = MCP251XFD_WriteSFR8(
      ...
      Config); // Write the Oscillator Register configuration
  if (Error != ERR_NONE)
    return Error; // If there is an error while writing the SFR then return the
                  // error

  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiCON + 1,
      &Config); // Read actual flags configuration of the RegMCP251XFD_CiCON
                // register (Second byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                  // then return the error
  Config &= ~(MCP251XFD_CAN_CiCON8_WFT_Mask |
  ...
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiINT_CONFIG + 1,
  ...
  if (interruptBusWakeUp)
    Config |= MCP251XFD_CAN_CiINT8_WAKIE; // Enable the Bus Wake Up Interrupt
  else
    Config &= ~MCP251XFD_CAN_CiINT8_WAKIE; // Disable the Bus Wake Up Interrupt
  return MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_CiINT_CONFIG + 1,
...
eERRORRESULT MCP251XFD_EnterSleepMode(MCP251XFD *pComp) {
  ...
  if (LastPS == MCP251XFD_DEVICE_SLEEP_NOT_CONFIGURED)
    return ERR__CONFIGURATION; // No configuration available to enter sleep mode
  if (LastPS != MCP251XFD_DEVICE_NORMAL_POWER_STATE)
    return ERR__ALREADY_IN_SLEEP; // Device already in sleep mode
  Error = MCP251XFD_RequestOperationMode(pComp, MCP251XFD_SLEEP_MODE,
                                         false); // Set Sleep mode
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_RequestOperationMode() then return the error
  pComp->InternalConfig &= ~MCP251XFD_DEV_PS_Mask;
  if ((pComp->InternalConfig & MCP251XFD_SFR_OSC8_LPMEN) > 0)
    pComp->InternalConfig |= MCP251XFD_DEV_PS_SET(
        MCP251XFD_DEVICE_LOWPOWER_SLEEP_STATE); // If Low Power Mode then the
                                                // device will be in low power
                                                // mode
  else
    pComp->InternalConfig |= MCP251XFD_DEV_PS_SET(
        MCP251XFD_DEVICE_SLEEP_STATE); // Else the device will be in sleep mode
  return ERR_NONE;
}
...
eERRORRESULT MCP251XFD_IsDeviceInSleepMode(MCP251XFD *pComp,
  ...
  *isInSleepMode = true;
  if (LastPS == MCP251XFD_DEVICE_LOWPOWER_SLEEP_STATE)
    return ERR__NOT_SUPPORTED; // Here if the device is in
                               // DEVICE_LOWPOWER_SLEEP_STATE, a simple assert
                               // of SPI CS exit the LPM then this function is
                               // not supported
  uint8_t Config;
  Error =
      MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_OSC_CONFIG,
  ...
  *isInSleepMode = ((Config & MCP251XFD_SFR_OSC8_OSCDIS) >
  ...
  if (*isInSleepMode == false) {
    pComp->InternalConfig &= ~MCP251XFD_DEV_PS_Mask;
    pComp->InternalConfig |= MCP251XFD_DEV_PS_SET(
        MCP251XFD_DEVICE_NORMAL_POWER_STATE); // If the function return is not
                                              // in sleep mode then refresh the
...
eERRORRESULT MCP251XFD_WakeUp(MCP251XFD *pComp,
  ...
  Error = MCP251XFD_ReadSFR8(
      ...
      &Config); // Read the Oscillator Register configuration (Here if the
                // device is in DEVICE_LOWPOWER_SLEEP_STATE, the reset already
                // happened because a simple assert of SPI CS exit the LPM)
  if (Error != ERR_NONE)
    return Error; // If there is an error while reading the SFR then return the
                  // error

  if (LastPS !=
      MCP251XFD_DEVICE_LOWPOWER_SLEEP_STATE) // In Sleep mode or a false normal
                                             // state, there is more to do
  {
    ...
    Error = MCP251XFD_WriteSFR8(
        ...
        Config); // Write the Oscillator Register configuration
    if (Error != ERR_NONE)
      return Error; // If there is an error while writing the SFR then return
                    // the error
  }
  if (fromState != NULL)
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureTimeStamp(MCP251XFD *pComp, bool enableTS,
  ...
  Error = MCP251XFD_WriteData(
      pComp, RegMCP251XFD_CiTSCON, &Config.Bytes[0],
      3); // Write new configuration to the CiTDC register (First 3-bytes only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_WriteData()
                  // then return the error

  uint8_t Flags;
  Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiINT_CONFIG,
  ...
  if (interruptBaseCounter)
    Flags |= MCP251XFD_CAN_CiINT8_TBCIE; // Add Time Base Counter Interrupt
                                         // Enable flag
  else
    Flags &= ~MCP251XFD_CAN_CiINT8_TBCIE; // Else clear the interrupt flag
  return MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_CiINT_CONFIG,
                             Flags); // Write the new flags configuration to the
                                     // CiINT register (Third byte only)
}
...
eERRORRESULT MCP251XFD_GetTimeStamp(MCP251XFD *pComp, uint32_t *value) {
  return MCP251XFD_ReadSFR32(pComp, RegMCP251XFD_CiTBC,
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureTEF(MCP251XFD *pComp, bool enableTEF,
  ...
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiCON + 2,
      &CiCONflags); // Read actual flags configuration of the RegMCP251XFD_CiCON
                    // register (Third byte only)
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR8()
                  // then return the error
  ...
  if (enableTEF)
    ...
    CiCONflags &=
        ~MCP251XFD_CAN_CiCON8_STEF; // Else Disable Transmit Event FIFO flag
  Error = MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_CiCON + 2,
  ...
  if (enableTEF) {
    ...
    uint8_t Size = MCP251XFD_CAN_TX_EVENTOBJECT_SIZE; // By default 1 element is
                                                      // 2x4-bytes in RAM
    Reg.CiTEFCON =
    ...
    if ((confTEF->InterruptFlags & MCP251XFD_FIFO_OVERFLOW_INT) > 0)
      Reg.CiTEFCON |=
          MCP251XFD_CAN_CiTEFCON_TEFOVIE; // Add Transmit Event FIFO Overflow
                                          // Interrupt Enable
    if ((confTEF->InterruptFlags & MCP251XFD_FIFO_EVENT_FIFO_FULL_INT) > 0)
      Reg.CiTEFCON |= MCP251XFD_CAN_CiTEFCON_TEFFIE; // Add Transmit Event FIFO
                                                     // Full Interrupt Enable
    if ((confTEF->InterruptFlags & MCP251XFD_FIFO_EVENT_FIFO_HALF_FULL_INT) > 0)
      Reg.CiTEFCON |=
          MCP251XFD_CAN_CiTEFCON_TEFHIE; // Add Transmit Event FIFO Half Full
                                         // Interrupt Enable
    if ((confTEF->InterruptFlags & MCP251XFD_FIFO_EVENT_FIFO_NOT_EMPTY_INT) > 0)
      Reg.CiTEFCON |=
          MCP251XFD_CAN_CiTEFCON_TEFNEIE; // Add Transmit Event FIFO Not Empty
                                          // Interrupt Enable

    Error = MCP251XFD_WriteData(
        pComp, RegMCP251XFD_CiTEFCON, &Reg.Bytes[0],
    ...
    if (confTEF->RAMInfos != NULL) {
      ...
      confTEF->RAMInfos->ByteInFIFO =
          (Size *
           ((uint8_t)confTEF->Size + 1)); // Total size of the TEF in RAM is 1
                                          // element size x Element Count
      confTEF->RAMInfos->RAMStartAddress =
          MCP251XFD_RAM_ADDR; // Set the start address of the TEF. Here the TEF
                              // is always the first in RAM
    }
  }
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureTXQ(MCP251XFD *pComp, bool enableTXQ,
  ...
  if (MCP251XFD_CAN_CiCON8_OPMOD_GET(CiCONflags) !=
    ...
    return ERR__NEED_CONFIG_MODE; // Device must be in Configuration Mode to
                                  // perform the configuration
  if (enableTXQ)
    CiCONflags |= MCP251XFD_CAN_CiCON8_TXQEN; // Add Enable Transmit Queue flag
  else
    CiCONflags &=
  ...
  if (enableTXQ) {
    ...
    Reg.CiTXQCON =
        MCP251XFD_CAN_CiTXQCON_PLSIZE_SET(confTXQ->Payload); // Add Payload Size
    ...
    if ((confTXQ->InterruptFlags & MCP251XFD_FIFO_TX_ATTEMPTS_EXHAUSTED_INT) >
        0)
      Reg.CiTXQCON |=
          MCP251XFD_CAN_CiTXQCON_TXATIE; // Add Transmit Attempts Exhausted
                                         // Interrupt Enable
    if ((confTXQ->InterruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_EMPTY_INT) > 0)
      Reg.CiTXQCON |= MCP251XFD_CAN_CiTXQCON_TXQEIE; // Add Transmit FIFO Empty
                                                     // Interrupt Enable
    if ((confTXQ->InterruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_NOT_FULL_INT) >
        0)
      Reg.CiTXQCON |= MCP251XFD_CAN_CiTXQCON_TXQNIE; // Add Transmit FIFO Not
                                                     // Full Interrupt Enable

    Error = MCP251XFD_WriteData(
        pComp, RegMCP251XFD_CiTXQCON, &Reg.Bytes[0],
    ...
    if (confTXQ->RAMInfos != NULL) {
      ...
      confTXQ->RAMInfos->ByteInFIFO =
          (Size *
           ((uint8_t)confTXQ->Size + 1)); // Total size of the TXQ in RAM is 1
                                          // element size x Element Count
...
eERRORRESULT MCP251XFD_ConfigureFIFO(MCP251XFD *pComp,
  ...
  if (confFIFO == NULL)
    return ERR__PARAMETER_ERROR;
#endif
  if ((confFIFO->Name == MCP251XFD_TEF) || (confFIFO->Name == MCP251XFD_TXQ) ||
  ...
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_GetActualOperationMode() then return the error
  ...
  if (confFIFO->Direction == MCP251XFD_TRANSMIT_FIFO) {
    Reg.CiFIFOCONm |=
        MCP251XFD_CAN_CiFIFOCONm_TXEN; // Transmit Event FIFO Time Stamp Enable
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_TX_ATTEMPTS_EXHAUSTED_INT) >
        0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TXATIE; // Add Transmit Attempts Exhausted
                                           // Interrupt Enable
    if ((confFIFO->ControlFlags & MCP251XFD_FIFO_AUTO_RTR_RESPONSE) > 0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_RTREN; // When a remote transmit is received,
                                          // Transmit Request (TXREQ) of the
                                          // FIFO will be set
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_EMPTY_INT) > 0)
      Reg.CiFIFOCONm |=
    ...
    if ((confFIFO->InterruptFlags &
      ...
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TFHRFHIE; // Add Transmit FIFO Half Empty
                                             // Interrupt Enable
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_NOT_FULL_INT) >
        0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TFNRFNIE; // Add Transmit FIFO Not Full
                                             // Interrupt Enable
  } else {
    ...
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_OVERFLOW_INT) > 0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_RXOVIE; // Add Transmit Attempts Exhausted
                                           // Interrupt Enable
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_RECEIVE_FIFO_FULL_INT) > 0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TFERFFIE; // Add Receive FIFO Full Interrupt
                                             // Enable
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_RECEIVE_FIFO_HALF_FULL_INT) >
        0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TFHRFHIE; // Add Receive FIFO Half Full
                                             // Interrupt Enable
    if ((confFIFO->InterruptFlags & MCP251XFD_FIFO_RECEIVE_FIFO_NOT_EMPTY_INT) >
        0)
      Reg.CiFIFOCONm |=
          MCP251XFD_CAN_CiFIFOCONm_TFNRFNIE; // Add Receive FIFO Not Empty
                                             // Interrupt Enable
  }
  ...
  if (confFIFO->RAMInfos != NULL) {
    ...
    confFIFO->RAMInfos->ByteInFIFO =
        (Size *
         ((uint8_t)confFIFO->Size + 1)); // Total size of the FIFO in RAM is 1
                                         // element size x Element Count
...
eERRORRESULT MCP251XFD_ConfigureFIFOList(MCP251XFD *pComp,
  ...
  uint8_t TEFcount = 0; // The TEF always start at address MCP251XFD_RAM_ADDR so
                        // it must be first
  for (size_t zTEF = 0; zTEF < count; zTEF++) {
    if (listFIFO[zTEF].Name == MCP251XFD_TEF) {
      ...
      //--- Configure TEF ---
      UseTmpRAMInfos =
          (listFIFO[zTEF].RAMInfos == NULL); // Is there a RAMInfos structure
                                             // attached to the configuration?
      if (UseTmpRAMInfos)
        listFIFO[zTEF].RAMInfos =
            &TmpRAMInfos; // If not RAMInfos structure attached, then set a
                          // temporary one
      Error = MCP251XFD_ConfigureTEF(pComp, true,
                                     &listFIFO[zTEF]); // Configure the TEF
      if (Error != ERR_NONE)
        return Error; // If there is an error while calling
                      // MCP251XFD_ConfigureTEF() then return the error
      TotalSize +=
          listFIFO[zTEF].RAMInfos->ByteInFIFO; // Add TEF size to the total
      if (UseTmpRAMInfos)
        listFIFO[zTEF].RAMInfos = NULL; // If not RAMInfos structure attached,
                                        // then unset the temporary one
    }
  }
  if (TEFcount == 0) {
  ...
  uint8_t TXQcount =
      0; // The TXQ always follow the TEF (if no TEF then it's the first) in RAM
  for (size_t zTXQ = 0; zTXQ < count; zTXQ++) {
    if (listFIFO[zTXQ].Name == MCP251XFD_TXQ) {
      ...
      //--- Configure TXQ --
      UseTmpRAMInfos =
          (listFIFO[zTXQ].RAMInfos == NULL); // Is there a RAMInfos structure
                                             // attached to the configuration?
      if (UseTmpRAMInfos)
        listFIFO[zTXQ].RAMInfos =
            &TmpRAMInfos; // If not RAMInfos structure attached, then set a
                          // temporary one
      Error = MCP251XFD_ConfigureTXQ(pComp, true,
                                     &listFIFO[zTXQ]); // Configure the TXQ
      if (Error != ERR_NONE)
        return Error; // If there is an error while calling
                      // MCP251XFD_ConfigureTXQ() then return the error
      listFIFO[zTXQ].RAMInfos->RAMStartAddress =
          MCP251XFD_RAM_ADDR + TotalSize; // Set start address
      TotalSize +=
          listFIFO[zTXQ].RAMInfos->ByteInFIFO; // Add TXQ size to the total
      if (UseTmpRAMInfos)
        listFIFO[zTXQ].RAMInfos = NULL; // If not RAMInfos structure attached,
                                        // then unset the temporary one
    }
  }
  if (TXQcount == 0) {
      ...
      if (listFIFO[z].Name == zFIFO) {
             ...
             clearFIFO++) // For each FIFO not listed between 2 FIFO
        {
          Error = MCP251XFD_ClearFIFOConfiguration(
              pComp, (eMCP251XFD_FIFO)zFIFO); // Clear FIFO configuration
          if (Error != ERR_NONE)
            return Error; // If there is an error while calling
                          // MCP251XFD_ClearFIFOConfiguration() then return the
                          // error
          TotalSize +=
              MCP251XFD_FIFO_MIN_SIZE; // Add min FIFO size. A FIFO cannot be
                                       // completely disable so, it takes the
                                       // minimum possible size
        }
        LastFIFO = zFIFO;
        //--- Configure FIFO --
        UseTmpRAMInfos =
            (listFIFO[z].RAMInfos == NULL); // Is there a RAMInfos structure
                                            // attached to the configuration?
        if (UseTmpRAMInfos)
          listFIFO[z].RAMInfos =
              &TmpRAMInfos; // If not RAMInfos structure attached, then set a
                            // temporary one
        Error =
            MCP251XFD_ConfigureFIFO(pComp, &listFIFO[z]); // Configure the FIFO
        if (Error != ERR_NONE)
          return Error; // If there is an error while calling
                        // MCP251XFD_ConfigureFIFO() then return the error
        listFIFO[z].RAMInfos->RAMStartAddress =
            MCP251XFD_RAM_ADDR + TotalSize; // Set start address
        TotalSize +=
            listFIFO[z].RAMInfos->ByteInFIFO; // Add FIFO size to the total
        if (UseTmpRAMInfos)
          listFIFO[z].RAMInfos = NULL; // If not RAMInfos structure attached,
                                       // then unset the temporary one
      }
    }
  }
...
eERRORRESULT MCP251XFD_ResetFIFO(MCP251XFD *pComp, eMCP251XFD_FIFO name) {
  ...
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_GetActualOperationMode() then return the error
  if (OpMode == MCP251XFD_CONFIGURATION_MODE)
    return ERR_NONE; // Device in Configuration Mode automatically reset the
                     // FIFO

  //--- Set Reset of the FIFO ---
  uint16_t Address = RegMCP251XFD_CiFIFOCONm_CONTROL +
                     (MCP251XFD_FIFO_REG_SIZE *
                      ((uint16_t)name - 1u)); // Select the address of the FIFO
  if (name == MCP251XFD_TEF)
    Address = RegMCP251XFD_CiTEFCON_CONTROL; // If it's the TEF then select its
                                             // address
  if (name == MCP251XFD_TXQ)
    Address = RegMCP251XFD_CiTXQCON_CONTROL; // If it's the TXQ then select its
                                             // address
  Error = MCP251XFD_WriteSFR8(
      pComp, Address,
  ...
  while (true) {
    ...
    if (MCP251XFD_TIME_DIFF(StartTime, pComp->fnGetCurrentms()) > 3)
      return ERR__DEVICE_TIMEOUT; // Wait at least 3ms. If timeout then return
                                  // the error
  }
  return ERR_NONE;
}
...
//=============================================================================
eERRORRESULT MCP251XFD_UpdateFIFO(MCP251XFD *pComp, eMCP251XFD_FIFO name,
                                  bool andFlush) {
  if (name >= MCP251XFD_FIFO_COUNT)
    return ERR__PARAMETER_ERROR;
...
eERRORRESULT MCP251XFD_FlushFIFO(MCP251XFD *pComp, eMCP251XFD_FIFO name) {
  ...
  //--- Set the flush ---
  uint16_t Address =
      RegMCP251XFD_CiTXREQ +
      ((uint16_t)name >> 3); // Select the good address of the TXREQ register
  return MCP251XFD_WriteSFR8(
      pComp, Address,
...
//=============================================================================
eERRORRESULT MCP251XFD_GetFIFOStatus(MCP251XFD *pComp, eMCP251XFD_FIFO name,
  ...
  //--- Set address of the FIFO ---
  uint16_t Address = RegMCP251XFD_CiFIFOSTAm_FLAGS +
                     (MCP251XFD_FIFO_REG_SIZE *
                      ((uint16_t)name - 1u)); // Select the address of the FIFO
  if (name == MCP251XFD_TEF)
    Address =
        RegMCP251XFD_CiTEFSTA_FLAGS; // If it's the TEF then select its address
  if (name == MCP251XFD_TXQ)
    Address =
        RegMCP251XFD_CiTXQSTA_FLAGS; // If it's the TXQ then select its address
  ...
  if (nextAddress != NULL) {
    ...
    Address = RegMCP251XFD_CiFIFOUAm +
              (MCP251XFD_FIFO_REG_SIZE *
               ((uint16_t)name - 1u)); // Select the address of the FIFO
    if (name == MCP251XFD_TEF)
      Address = RegMCP251XFD_CiTEFUA; // If it's the TEF then select its address
    if (name == MCP251XFD_TXQ)
      Address = RegMCP251XFD_CiTXQUA; // If it's the TXQ then select its address
    //--- Get FIFO status ---
    Error = MCP251XFD_ReadSFR32(pComp, Address,
                                nextAddress); // Read FIFO user address
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_ReadSFR32()
                    // then return the error
  }
  ...
  if (nextIndex != NULL) {
    ...
    Address = RegMCP251XFD_CiFIFOSTAm_FIFOCI +
              (MCP251XFD_FIFO_REG_SIZE *
               ((uint16_t)name - 1u)); // Select the address of the FIFO
    if (name == MCP251XFD_TXQ)
      Address = RegMCP251XFD_CiTXQSTA_TXQCI; // If it's the TXQ then select its
                                             // address
...
eERRORRESULT MCP251XFD_ClearFIFOConfiguration(MCP251XFD *pComp,
  ...
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ResetFIFO()
                  // then return the error
#ifndef __cplusplus
...
eERRORRESULT MCP251XFD_SetFIFOinterruptConfiguration(
    MCP251XFD *pComp, eMCP251XFD_FIFO name,
  ...
  //--- Set address of the FIFO ---
  uint16_t Address = RegMCP251XFD_CiFIFOCONm_CONFIG +
                     (MCP251XFD_FIFO_REG_SIZE *
                      ((uint16_t)name - 1u)); // Select the address of the FIFO
  if (name == MCP251XFD_TEF)
    Address =
        RegMCP251XFD_CiTEFCON_CONFIG; // If it's the TEF then select its address
  if (name == MCP251XFD_TXQ)
    Address =
        RegMCP251XFD_CiTXQCON_CONFIG; // If it's the TXQ then select its address
  ...
  if (((interruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_NOT_FULL_INT) > 0) ||
    ...
    Config |=
        MCP251XFD_CAN_CiFIFOCONm8_TFNRFNIE; // Transmit/Receive FIFO Not
                                            // Full/Not Empty Interrupt Enable
  if (((interruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_HALF_EMPTY_INT) > 0) ||
    ...
    Config |=
        MCP251XFD_CAN_CiFIFOCONm8_TFHRFHIE; // Transmit/Receive FIFO Half
                                            // Empty/Half Full Interrupt Enable
  if (((interruptFlags & MCP251XFD_FIFO_TRANSMIT_FIFO_EMPTY_INT) > 0) ||
      ((interruptFlags & MCP251XFD_FIFO_RECEIVE_FIFO_FULL_INT) > 0))
    Config |= MCP251XFD_CAN_CiFIFOCONm8_TFERFFIE; // Transmit/Receive FIFO
                                                  // Empty/Full Interrupt Enable
  if (((interruptFlags & MCP251XFD_FIFO_OVERFLOW_INT) > 0))
    Config |= MCP251XFD_CAN_CiFIFOCONm8_RXOVIE; // Overflow Interrupt Enable
  if (((interruptFlags & MCP251XFD_FIFO_TX_ATTEMPTS_EXHAUSTED_INT) > 0))
    Config |= MCP251XFD_CAN_CiFIFOCONm8_TXATIE; // Transmit Attempts Exhausted
                                                // Interrupt Enable
  return MCP251XFD_WriteSFR8(
      pComp, Address, Config); // Write new configuration to the FIFO's register
}
...
//=============================================================================
eERRORRESULT MCP251XFD_ConfigureDeviceNetFilter(MCP251XFD *pComp,
  ...
  CiCONflags |=
      MCP251XFD_CAN_CiCON8_DNCNT_SET(filter); // Set new filter configuration
  return MCP251XFD_WriteSFR8(
      pComp, RegMCP251XFD_CiCON,
      CiCONflags); // Write the new flags configuration to the
...
eERRORRESULT MCP251XFD_ConfigureFilter(MCP251XFD *pComp,
  ...
  uint16_t AddrFLTCON = RegMCP251XFD_CiFLTCONm +
  ...
  if (confFilter->EnableFilter) {
    ...
    FltObj.CiFLTOBJm = MCP251XFD_MessageIDtoObjectMessageIdentifier(
        confFilter->AcceptanceID,
        (confFilter->Match != MCP251XFD_MATCH_ONLY_SID), UseSID11);
    if (confFilter->Match == MCP251XFD_MATCH_ONLY_EID)
      FltObj.CiFLTOBJm |=
          MCP251XFD_CAN_CiFLTOBJm_EXIDE; // If filter match only messages with
                                         // extended identifier set the bit
                                         // EXIDE
    ...
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_WriteSFR32()
                    // then return the error
    ...
    if (confFilter->Match != MCP251XFD_MATCH_SID_EID)
      FltMask.CiMASKm |=
          MCP251XFD_CAN_CiMASKm_MIDE; // If filter match only messages with
                                      // standard or extended identifier set the
                                      // bit MIDE
...
eERRORRESULT MCP251XFD_ConfigureFilterList(MCP251XFD *pComp,
  ...
  Error =
      MCP251XFD_ConfigureDeviceNetFilter(pComp, filter); // Configure the DNCNT
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling
                  // MCP251XFD_ConfigureDeviceNetFilter() then return the error
  ...
  for (int32_t zFilter = 0; zFilter < MCP251XFD_FILTER_COUNT; zFilter++) {
    for (size_t z = 0; z < count; z++) {
      if (listFilter[z].Filter == zFilter) {
        ...
        if (Error != ERR_NONE)
          return Error; // If there is an error while calling
                        // MCP251XFD_ConfigureFilter() then return the error
      }
    }
    if (Modified == false) // Filter not listed
    {
      Error = MCP251XFD_DisableFilter(
          pComp,
          (eMCP251XFD_Filter)zFilter); // Disable the Filter configuration
      if (Error != ERR_NONE)
        return Error; // If there is an error while calling
                      // MCP251XFD_DisableFilter() then return the error
    }
    Modified = false;
  }
...
eERRORRESULT MCP251XFD_DisableFilter(MCP251XFD *pComp, eMCP251XFD_Filter name) {
  ...
  uint16_t AddrFLTCON =
      RegMCP251XFD_CiFLTCONm + name; // Select the address of the FLTCON
  Error = MCP251XFD_ReadSFR8(
      pComp, AddrFLTCON,
      &FilterConf.CiFLTCONm); // Read actual flags configuration of the
                              // RegMCP251XFD_CiFLTCONm register
...
MCP251XFD_ConfigureInterrupt(MCP251XFD *pComp,
  ...
  if (Error != ERR_NONE)
    return Error; // If there is an error while calling MCP251XFD_ReadSFR16()
                  // then return the error
  return MCP251XFD_WriteSFR16(
      pComp, RegMCP251XFD_CiINT_CONFIG,
      interruptsFlags); // Write new interrupt configuration (The 2 MSB bytes)
}
...
MCP251XFD_GetInterruptEvents(MCP251XFD *pComp,
  ...
  return MCP251XFD_ReadSFR16(
      pComp, RegMCP251XFD_CiINT_FLAG,
      (uint16_t *)interruptsFlags); // Read interrupt flags (The 2 LSB bytes)
}
...
//=============================================================================
eERRORRESULT
MCP251XFD_GetCurrentInterruptEvent(MCP251XFD *pComp,
                                   eMCP251XFD_InterruptFlagCode *currentEvent) {
  return MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiVEC_ICODE,
      (uint8_t *)
...
eERRORRESULT MCP251XFD_GetCurrentReceiveFIFONameAndStatusInterrupt(
    MCP251XFD *pComp, eMCP251XFD_FIFO *name, setMCP251XFD_FIFOstatus *flags) {
#ifdef CHECK_NULL_PARAM
  if (name == NULL)
    return ERR__PARAMETER_ERROR;
#endif
  ...
  *name = MCP251XFD_NO_FIFO;
  uint8_t RxCode = 0;
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiVEC_RXCODE,
  ...
  if ((RxCode > MCP251XFD_FIFO31) && (RxCode != MCP251XFD_NO_INTERRUPT))
    return ERR__UNKNOWN_ELEMENT; // Only FIFO1 to 31 and no Interrupt code
                                 // possible so the rest is not possible
  ...
  *name = (eMCP251XFD_FIFO)RxCode; // Save the current FIFO name interrupt
  ...
  if ((*name != MCP251XFD_NO_FIFO) && (flags != NULL))
    return MCP251XFD_GetFIFOStatus(pComp, *name,
                                   flags); // Get status flags of the FIFO
  return ERR_NONE;
}
...
eERRORRESULT MCP251XFD_GetCurrentTransmitFIFONameAndStatusInterrupt(
  ...
  *name = MCP251XFD_NO_FIFO;
  uint8_t TxCode = 0;
  Error = MCP251XFD_ReadSFR8(
      pComp, RegMCP251XFD_CiVEC_TXCODE,
  ...
  if ((TxCode > MCP251XFD_FIFO31) && (TxCode != MCP251XFD_NO_INTERRUPT))
    return ERR__UNKNOWN_ELEMENT; // Only FIFO0 (TXQ) to 31 and no Interrupt code
                                 // possible so the rest is not possible
...
//=============================================================================
eERRORRESULT MCP251XFD_ClearFIFOEvents(MCP251XFD *pComp, eMCP251XFD_FIFO name,
                                       uint8_t events) {
  if (name >= MCP251XFD_FIFO_COUNT)
    return ERR__PARAMETER_ERROR;
  ...
  if (((events & MCP251XFD_TXQ_ATTEMPTS_EXHAUSTED) > 0) ||
    ...
    Status &= ~MCP251XFD_CAN_CiFIFOSTAm8_TXATIF; // Transmit & TXQ: Clear
                                                 // Attempts Exhausted event
  if (((events & MCP251XFD_TXQ_BUS_ERROR) > 0) ||
      ((events & MCP251XFD_TX_FIFO_BUS_ERROR) > 0))
    Status &=
        ~MCP251XFD_CAN_CiFIFOSTAm8_TXERR; // Transmit & TXQ: Clear Error
                                          // Detected During Transmission event
  if (((events & MCP251XFD_TXQ_ARBITRATION_LOST) > 0) ||
      ((events & MCP251XFD_TX_FIFO_ARBITRATION_LOST) > 0))
    Status &=
        ~MCP251XFD_CAN_CiFIFOSTAm8_TXLARB; // Transmit & TXQ: Clear Message Lost
                                           // Arbitration event
  if (((events & MCP251XFD_TXQ_STATUS_MASK) > 0) ||
      ((events & MCP251XFD_TX_FIFO_STATUS_MASK) > 0))
    Status &= ~MCP251XFD_CAN_CiFIFOSTAm8_TXABT; // Transmit & TXQ: Clear Message
                                                // Aborted event
  return MCP251XFD_WriteSFR8(pComp, Address,
                             Status); // Write new FIFO status flags
}
...
eERRORRESULT MCP251XFD_GetReceiveInterruptStatusOfAllFIFO(
  ...
  if (overflowStatus != NULL) {
    Error = MCP251XFD_ReadSFR32(
        pComp, RegMCP251XFD_CiRXOVIF,
        (uint32_t *)overflowStatus); // Read the Receive overflow interrupt
                                     // status register
    if (Error != ERR_NONE)
      return Error; // If there is an error while calling MCP251XFD_ReadSFR32()
                    // then return the error
...
eERRORRESULT MCP251XFD_GetTransmitReceiveErrorCountAndStatus(
    MCP251XFD *pComp, uint8_t *transmitErrorCount, uint8_t *receiveErrorCount,
    eMCP251XFD_TXRXErrorStatus *status) {
  eERRORRESULT Error;
  if (transmitErrorCount != NULL) {
    Error = MCP251XFD_ReadSFR8(
        pComp, RegMCP251XFD_CiTREC_TEC,
  ...
  if (receiveErrorCount != NULL) {
    Error =
        MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiTREC_REC,
  ...
  if (status != NULL) {
    Error = MCP251XFD_ReadSFR8(
        pComp, RegMCP251XFD_CiTREC_STATUS,
...
MCP251XFD_GetBusDiagnostic(MCP251XFD *pComp,
  ...
  if (busDiagnostic0 != NULL) {
    Error = MCP251XFD_ReadSFR32(pComp, RegMCP251XFD_CiBDIAG0,
  ...
  if (busDiagnostic1 != NULL) {
    Error = MCP251XFD_ReadSFR32(pComp, RegMCP251XFD_CiBDIAG1,
...
eERRORRESULT MCP251XFD_ClearBusDiagnostic(MCP251XFD *pComp,
  ...
  if (clearBusDiagnostic0) {
    Error = MCP251XFD_WriteSFR32(pComp, RegMCP251XFD_CiBDIAG0,
  ...
  if (clearBusDiagnostic1) {
    Error = MCP251XFD_WriteSFR32(pComp, RegMCP251XFD_CiBDIAG1,
...
eERRORRESULT MCP251XFD_ResetDevice(MCP251XFD *pComp) {
  ...
  if ((pComp->DriverConfig & MCP251XFD_DRIVER_SAFE_RESET) > 0) {
    ...
    Error = MCP251XFD_RequestOperationMode(
        pComp, MCP251XFD_CONFIGURATION_MODE,
        true); // Request a configuration mode and wait until device is this
               // mode
    if (Error != ERR_OK)
      return Error; // If there is an error while calling
                    // MCP251XFD_RequestOperationMode() then return the error
  ...
  //-- Perform Reset --------------------------------------------------------
  return pComp->fnSPI_Transfer(pComp->InterfaceDevice, pComp->SPI_ChipSelect,
                               &Buffer[0], NULL,
...
eERRORRESULT __MCP251XFD_TestRAM(MCP251XFD *pComp) {
  ...
  for (uint16_t Address = MCP251XFD_RAM_ADDR;
       Address < (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE); Address += 4) {
    Error = MCP251XFD_WriteRAM32(pComp, Address,
                                 0x55555555); // Write 0x55555555 at address
    if (Error != ERR_NONE)
      return Error; // If there is an error while writing the RAM address then
                    // return the error
    ...
    if (Result != 0x55555555)
      return ERR__RAM_TEST_FAIL; // If data read is not 0x55555555 then return
                                 // an error

    Error = MCP251XFD_WriteRAM32(pComp, Address,
                                 0xAAAAAAAA); // Write 0xAAAAAAAA at address
    if (Error != ERR_NONE)
      return Error; // If there is an error while writing the RAM address then
                    // return the error
    Error = MCP251XFD_ReadRAM32(pComp, Address, &Result); // Read again the data
    ...
    if (Result != 0xAAAAAAAA)
      return ERR__RAM_TEST_FAIL; // If data read is not 0xAAAAAAAA then return
                                 // an error
  }
  return ERR_NONE;
}
...
uint32_t MCP251XFD_MessageIDtoObjectMessageIdentifier(uint32_t messageID,
  ...
  {
    ResultOMI = ((messageID >> MCP251XFD_EID_Size) & MCP251XFD_SID_Mask) |
                ((messageID & MCP251XFD_EID_Mask) << MCP251XFD_SID_Size);
    if (UseSID11)
      ResultOMI |=
          (messageID & (1 << (MCP251XFD_EID_Size + MCP251XFD_SID_Size)));
  } else {
...
//=============================================================================
uint32_t MCP251XFD_ObjectMessageIdentifierToMessageID(uint32_t objectMessageID,
                                                      bool extended,
                                                      bool UseSID11) {
  ...
  if (extended) // Message use extended ID?
  {
    ResultID = ((objectMessageID >> MCP251XFD_SID_Size) & MCP251XFD_EID_Mask) |
               ((objectMessageID & MCP251XFD_SID_Mask) << MCP251XFD_EID_Size);
    if (UseSID11)
      ResultID |=
          (objectMessageID & (1 << (MCP251XFD_EID_Size + MCP251XFD_SID_Size)));
  } else {
...
uint8_t MCP251XFD_PayloadToByte(eMCP251XFD_PayloadSize payload) {
  //  const uint8_t PAYLOAD_TO_VALUE[PAYLOAD_COUNT] = {8, 12, 16, 20, 24, 32,
  //  48, 64}; if (payload < PAYLOAD_COUNT) return PAYLOAD_TO_VALUE[payload];
  //  return 0;
  switch (payload) {
